...
| 错题订正 | 意图识别触发（说「订正这题」或直接贴题目和作答问对错均可）：题目与作答图片自动转录 → 正确答案三级基线（官方答案/模型解答经学生确认/待确认）→ 定位第一个分歧步骤引导订正 → 确认卡片入错题本；未订正条目自动注入下次会话跟进 |
...
| 复习提醒 | 状态栏徽标 + 每日温和 Notice；复查用 AI 变式题（不重做原题） |
