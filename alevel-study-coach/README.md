...
| 口语训练 | 教练「雅思口语训练」：A 完整模考（Part1→2→3）/ B 单 Part 专项 / C 自由陪练 / D 讨论与复盘；延迟限量纠错、首次校准定阶段目标（循序渐进到 7.5）、Band 7 证据式评分（区间不讨好）；终训评分进 `雅思/口语记录.md` 与写作趋势并排，口语错题进现有错题本（SP/GR/VX），高分表达进积累库；当前为文字模式（语音链路 P5b 建设中，P 维度暂标 -） |
...
- 雅思听力/阅读不做批改打卡（线下主学习范畴）；口语语音链路（ASR/TTS）建设中，当前为文字模式
