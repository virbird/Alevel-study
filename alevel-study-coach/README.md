...
| 功能 | 说明 |
|---|---|
| 作文工作流 | 「新建作文笔记」→ 粘到「## 原文」→「批改当前作文」：ys.md 六段输出回填笔记 |
| 分数入库 | 批改末尾 JSON 自动提取总分与 TR/CC/LR/GRA 进 frontmatter，解析失败不阻塞 |
| 趋势与短板 | 雅思页签按日期展示分数，高亮短板维度与距 7.5 目标差距 |
| 表达积累库 | 批改提取高分表达自动入库（去重），SM-2 简化间隔 1→3→7→14→30→60 天 |
| 表达造句抽查 | 到期表达逐个造句，AI 判定后自动升档/重置，封顶再通过→已掌握 |
| 与 A-Level 联动 | 雅思页签并排展示学科 DV/CL/LK/E 表达码趋势（同一能力，正反馈） |
...
├── 建议/            # 弱点建议卡片（同意后生成学习建议）
├── 雅思/
│   ├── 作文/        # 每篇一个笔记：原文 + 批改 + 分数 frontmatter
...
| UT: ielts | 批改结果解析容错、作文笔记、分数趋势、表达去重与间隔调度 |
...
- 聊天非流式（回复整段出现）；不支持图片
- 牛剑进阶角与 Dashboard 全量图表在 Phase 4
