...
| 错题↔会话回链 | 每条错题记录发生时的会话；记录中心/复习队列一键打开原会话，「去练」预填当时的错误 + 原会话链接 |
| 掌握度进度徽章 | 状态旁显示距下一状态的距离（如「1 次抽查即稳定」）；行内「去练」按钮跳教练页签预填请求（术语抽查/错题重做/变式） |
| 弱点画像（L2） | 按科目生成带台账 ID 回链的压缩画像，结题后自动刷新；未消除条目多时注入画像替代全表（省 token）；「人工备注」节重新生成时保留 |
| 复习单 | 一键把四队到期项编译成一页可打印笔记（术语闪卡/错题重做/表达造句）；指纹漂移检测提示过期重生成 |
| 趋势图 | 本地 SVG（零依赖）：写作/口语总分折线、记录中心近 30 天失分码条形分布 |
| 复盘对照 | 英文复盘步骤后「对照」按钮，词级 diff 展示学生自己两版（红删绿增） |
